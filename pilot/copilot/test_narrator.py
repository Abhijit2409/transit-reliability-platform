...
# --------------------------------------------------------------------------- #
# Deterministic post-generation anchor enforcement (L02 / L04 guarantee)
# These prove that even when the MODEL omits anchors, the final answer_text
# contains them — independent of prompt compliance.
# --------------------------------------------------------------------------- #

class OmittingClient:
    """A model that returns prose deliberately MISSING the required anchors."""
...
def test_l02_missing_anchors_are_appended():
    """Model omits '099' and '84.21'; final answer_text must contain both."""
    result = QueryResult(
        columns=("route_short_name", "reliability_band", "reliability_score"),
        rows=(("099", "Watch", 84.21),),
        row_count=1,
    ...
    # paraphrased answer that drops the canonical token AND the score
    client = OmittingClient("The Broadway B-Line is on the watch list.")
    out = narrate("What reliability band is the 99 B-Line in?", result.sql, result, client=client)
    assert out.status is NarrationStatus.OK
    assert "099" in out.answer_text
    assert "84.21" in out.answer_text


def test_l04_missing_route_token_is_appended():
    """Model omits 'R4'; final answer_text must contain it."""
    result = QueryResult(
        columns=("route_short_name", "route_long_name", "intervention_priority_score"),
        rows=(("R4", "41st Avenue", 52.42),),
        row_count=1,
    ...
    # answer mentions the long name and number but NOT the identifier
    client = OmittingClient("41st Avenue has a priority score of 52.42.")
    out = narrate("What is the intervention priority score for the 41st Avenue route?",
                  result.sql, result, client=client)
    assert out.status is NarrationStatus.OK
    assert "R4" in out.answer_text


def test_anchors_not_duplicated_when_model_already_includes_them():
    """If the model already echoed every anchor, nothing is appended."""
    result = QueryResult(
        columns=("route_short_name", "reliability_score"),
        rows=(("099", 84.21),),
        row_count=1,
        sql="SELECT ...",
    )
    client = OmittingClient("Route 099 has a reliability score of 84.21.")
    out = narrate("q", result.sql, result, client=client)
    assert out.status is NarrationStatus.OK
    assert "Result values:" not in out.answer_text   # no redundant suffix
    assert out.answer_text == "Route 099 has a reliability score of 84.21."


def test_partial_missing_only_appends_the_missing_one():
    """Model includes the route but drops the number -> only the number appended."""
    result = QueryResult(
        columns=("route_short_name", "reliability_score"),
        rows=(("R4", 77.6),),
        row_count=1,
    ...
    client = OmittingClient("R4 is in the watch band.")
    out = narrate("q", result.sql, result, client=client)
    assert "R4" in out.answer_text
    assert "77.6" in out.answer_text
    assert "Result values: 77.6." in out.answer_text

...
# --- rate-limit safety (configurable via env) ------------------------------ #
# Delay between questions (seconds) to stay under tokens-per-minute limits.
_INTER_QUESTION_DELAY_S = float(os.environ.get("TIC_EVAL_DELAY_S", "3"))
# Max retry attempts for a question whose failure was caused by a 429.
_MAX_RETRIES = int(os.environ.get("TIC_EVAL_MAX_RETRIES", "5"))
# Base backoff (seconds); doubles each retry: base, 2*base, 4*base, ...
_BACKOFF_BASE_S = float(os.environ.get("TIC_EVAL_BACKOFF_BASE_S", "5"))
# Cap on any single backoff wait.
_BACKOFF_MAX_S = float(os.environ.get("TIC_EVAL_BACKOFF_MAX_S", "60"))

# Only these pipeline statuses are eligible for rate-limit retry. Validation
# failures, low-confidence, refusals, execution errors are terminal results,
# never retried.
_RETRYABLE_STATUSES = (
    PipelineStatus.GENERATION_ERROR,
    PipelineStatus.NARRATION_ERROR,
)

# Substrings (case-insensitive) that identify a 429 / rate-limit cause.
_RATE_LIMIT_MARKERS = ("rate limit", "rate_limit_exceeded", "429", "tokens per minute")


def _is_rate_limited(result: PipelineResult) -> bool:
    """True only if the failure was a retryable status caused by a 429."""
    if result.status not in _RETRYABLE_STATUSES:
        return False
    blob = " ".join(
        str(x).lower()
        for x in (result.detail, result.execution_error, result.narration_error)
        if x
    )
...
# Numeric anchors: a number NOT immediately followed by a letter, so ordinals
# and street numbers embedded in prose ('41st Avenue', '22nd St') are excluded
# and only standalone metric values (scores, rates, counts) are captured.
_NUM_RE = re.compile(r"\d[\d,]*\.?\d*(?![\d.]*[A-Za-z])")
...
def _run_question(q: dict, db_path: str | None) -> EvalRow:
    ...
    while True:
        ...
        # Retry ONLY when the failure was a 429-caused generation/narration error.
        if _is_rate_limited(result) and retry_count < _MAX_RETRIES:
            wait = min(_BACKOFF_BASE_S * (2 ** retry_count), _BACKOFF_MAX_S)
            retry_count += 1
            print(
                f"  [retry {retry_count}/{_MAX_RETRIES}] {qid}: rate-limited "
                f"({result.status.value}); waiting {wait:.0f}s before retry",
                flush=True,
            )
            time.sleep(wait)
            continue
...
def print_summary(summary: EvalSummary) -> None:
    ...
    _retries = sum(r.retry_count for r in summary.rows)
    if _retries:
        _retried_ids = [r.qid for r in summary.rows if r.retry_count]
        print(f"Rate-limit retries: {_retries}  (questions: {', '.join(_retried_ids)})")
...
def run_eval(db_path: str | None = None) -> EvalSummary:
    ...
    for i, q in enumerate(questions):
        ...
        # Pace requests to stay under tokens-per-minute limits. Skip the wait
        # after the final question. Delay is configurable (TIC_EVAL_DELAY_S).
        if _INTER_QUESTION_DELAY_S > 0 and i < last:
            time.sleep(_INTER_QUESTION_DELAY_S)
    return summary
